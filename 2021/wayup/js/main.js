 /* 
    Задание 1:

    Вам необходимо поделиться информацией о вашем родном городе. Все данные необходимо записать в отдельную переменную.
    Информация о городе: 
    - Название города (строка)
    - В какой стране находится этот город (строка)
    - Численность населения (число)
    - Есть ли футбольный стадион (boolean [ true(да) / false(нет) ])
*/
console.log('Задание 1:')
let city = 'Екатеринбург';
console.log('Город:' + ' ' + city);
let country = 'Россия';
console.log('Страна:' + ' ' + country);
let populationSize = 1495066;
console.log('Численность населения:' + ' ' + populationSize);
let footballStadium = true;
console.log('Футбольный стадион:' + ' ' + footballStadium);
console.log(' ');
/* 
    Задание 2:
    
    Напишите скрипт, который находит площадь прямоугольника

    - высота 40см
    - ширина 70см

    ps: каждая сущность должна находиться в отдельной переменной
*/
console.log('Задание 2:')
let height = 40;
console.log('высота = ' + height + 'см');
let width = 70;
console.log('Ширина = ' + width + 'см')
S = height * width;
console.log('Площадь прямоугольника равна ' + S + 'кв.см')
console.log(' ');

/* 
    Задание 3:
    
    Два автомобиля одновременно выехали навстречу друг другу из двух городов и встретились через 2 часа.
    Первый ехал со скоростью 95км/ч, а второй 114км/ч.

    Цель: Выяснить на каком расстоянии находятся города друг от друга и после всех вычеслений записать результат в переменную.

    Исходные данные: 
    time = 2;
    speedOfFirst = 95;
    speedOfSecond = 114;

*/
console.log('Задание 3:');
let time = 2;
console.log('Время, через которое встретились два автомобиля, выехав друг другу на встречу: ' + time + ' часа.');
let speedOfFirst = 95;
console.log('Скорость одного автомобиля: ' + speedOfFirst + ' км/ч');
let speedOfSecond = 114;
console.log('Скорость второго автомобля: ' + speedOfSecond + ' км/ч');
let distanceBetweenCity = (time * speedOfFirst) + (time * speedOfSecond);
console.log('Расстояние между городами, из которых одновременно выехали два автомобиля на встречу друг другу равняется: ' + distanceBetweenCity + ' км');
console.log(' ');

/* 
    Задание 4:
    
    Перед вами код:
    const randomNumber = Math.floor(Math.random() * 100);

    Этот код при каждом обновлении страницы генерирует случайное число и записывает его в переменную randomNumber.

    Напишите условную конструкцию, со следующими данными:
    - если randomNumber меньше 20, то выведите в консоль сообщение : "randomNumber меньше 20"
    - если randomNumber больше 50, то выведите в консоль сообщение : "randomNumber больше 50"
    - если ни один из вариантов не совпал, то выведите в консоль сообщение : "randomNumber больше 20, и меньше 50"
*/
console.log('Задание 4:');
const randomNumber = Math.floor(Math.random() * 100);
console.log('При обновлении страницы выпало число: ' + randomNumber);
if (randomNumber < 20){
    console.log("randomNumber меньше 20");}
    else if (randomNumber > 50){
        console.log("randomNumber больше 50");
    }
    else {console.log("randomNumber больше 20, и меньше 50");
};
console.log(' ');
/* 
    Задание 5:
    
    Условную конструкцию из задания 4, перепишите с помощью Switch Case
*/
console.log('Задание 5 (применить к заданию 4 конструкцию "Switch Case"):');
console.log('При обновлении страницы выпало число: ' + randomNumber);
switch(20 < randomNumber < 50) {
    case randomNumber < 20: 
        console.log("randomNumber меньше 20");
        break;
    case randomNumber > 50:
        console.log("randomNumber больше 50");
        break;
    default:
        console.log("randomNumber больше 20, и меньше 50");
        break;
}